...
def extract_count(text):
    pattern = r'statistic wireless96Nodes\.host(\d+)\.aodv Data_packets_Sent:stats\nfield count (\d+)'
...
with open(cwd + '//General-#0.sca', 'r') as file:
    text = file.read()
    matches = re.findall(r'statistic wireless96Nodes\.host\d+\.aodv Data_packets_Sent:stats.*?attr interpolationmode none', text, re.DOTALL)
