...
def extract_count(text):
    pattern = r'statistic wireless24Nodes\.host(\d+)\.aodv Control_packets:stats\nfield count (\d+)'
...
with open(cwd + '//General-#0.sca', 'r') as file:
    text = file.read()
    matches = re.findall(r'statistic wireless24Nodes\.host\d+\.aodv Control_packets:stats.*?attr interpolationmode none', text, re.DOTALL)
