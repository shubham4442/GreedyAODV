...
namespace inet
{
    namespace aodv
    {
        ...
        void Aodv::initialize(int stage)
        {
            ...
            ologfile.open("Logs_AoDV.txt", std::ios::trunc);
            ...
            else if (stage == INITSTAGE_ROUTING_PROTOCOLS)
            {
                ...
                // registering signal
                RREQ_traffic = registerSignal("RREQ_traffic");
            }
        ...
        const Ptr<Rreq> Aodv::createRREQ(const L3Address &destAddr)
        {
            ...
            //set the current node position and desired search radius from q-Table.
            Coord selfPosition = mobility->getCurrentPosition();
            // Access q-Table for the best radius
            double searchRadius = 5000;
            rreqPacket->setSearchRadius(searchRadius);
            rreqPacket->setOriginatorPos(selfPosition);
        ...
        void Aodv::handleRREQ(const Ptr<Rreq> &rreq, const L3Address &sourceAddr, unsigned int timeToLive)
        {
            ...
            Coord originatorPos = rreq->getOriginatorPos();
            double searchRadius = rreq->getSearchRadius();
            Coord selfPosition = mobility->getCurrentPosition();
            double curDistance = (selfPosition - originatorPos).length();
        ...
        L3Address Aodv::findGreedyRoutingNextHop(const L3Address &destination, GreedyOption *greedyOption)
        {
            ...
            //std::ofstream ologfile;
            //ologfile.open("Logs_AoDV.txt", std::ios::app);
            //ologfile << "Neigbours for " << selfAddress << "is following :" << std::endl;
            for (auto &neighborAddress : neighborAddresses)
            {
                ...
                //ologfile << neighborAddress << "at distance " << neighborDistance << std::endl;
        ...
        INetfilter::IHook::Result Aodv::datagramPreRoutingHook(Packet *datagram)
        {
            ...
            else
            {
                ...
                std::ofstream ologfile;
                ologfile.open("Logs_AoDV.txt", std::ios::app);
                //std::string ologline = "Logging method datagramPreRoutingHook for host number " ;
                //ologfile << ologline << getSelfIPAddress() << std::endl;
                ...
                if (isActive && !route->getNextHopAsGeneric().isUnspecified())
                {
                    ...
                    ologfile << "I am pre routing, for "<< getSelfIPAddress() << "active route found, this means the route earlier found by AODV, and is still saved" << std::endl;
                    ologfile.close();
                    return ACCEPT;
                }
                else
                {
                    ...
                    ologfile << "starting AODV route search : either greedy route not found or mode was AODV" << std::endl;
                    ...
                    if (!hasOngoingRouteDiscovery(destAddr))
                    {
                        ...
                        ologfile << "starting route discovery" << std::endl;
                        if (isInactive)
